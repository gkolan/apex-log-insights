...
function writeHash(state) {
  ...
  if (state.view === "evidence" && state.query)
  ...
  if (state.view === "evidence" && state.offset > 0)
      ...
      <div class="topbar-row topbar-row-primary">
        <div class="topbar-left">
          <div class="topbar-logo" aria-hidden="true">
            <img src="./icons/icon-active.svg" alt="" width="32" height="32" />
          </div>
...
export function createShell({ mount, report, sourceUrl }) {
  ...
  function jumpToEvidence(line) {
    ...
    if (state.line) state.query = `log:${state.line}`;
    state.offset = 0;
    if (!writeHash(state)) rerender();
  ...
  function bindCopy(root) {
    ...
    const copy = async (selector, emptyMessage, successLabel) => {
      ...
      try {
        await copyText(text);
        if (status)
          status.textContent = `Copied ${rows.length} ${successLabel} line(s).`;
      } catch {
    ...
    if (copyVisibleBtn) {
      ...
      copyVisibleBtn.addEventListener("click", () =>
        copy(".rawLine", "No visible lines to copy.", "visible"),
      );
    }
    if (copyMatchedBtn) {
      copyMatchedBtn.disabled =
        root.querySelectorAll(".rawLine.isMatch").length === 0;
      copyMatchedBtn.addEventListener("click", () =>
        copy(".rawLine.isMatch", "No matched lines to copy.", "matched"),
      );
    }
  }
  ...
  function bind() {
    mount.querySelectorAll("[data-nav-view]").forEach((button) => {
      button.addEventListener("click", () =>
        setView(button.getAttribute("data-nav-view")),
      );
    });

    mount.querySelectorAll("[data-toggle-section]").forEach((button) => {
      button.addEventListener("click", () =>
        toggleExpanded(button.getAttribute("data-toggle-section")),
      );
    });
    ...
    mount.querySelectorAll("[data-evidence-offset]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        state.offset = Math.max(
          0,
          Number(button.getAttribute("data-evidence-offset") || 0) || 0,
        );
        if (!writeHash(state)) rerender();
      });
    });
    ...
    mount.querySelectorAll("[data-export-csv]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const ts = timestamp();
        if (tableType === "soql") {
          const rows = report.data.soql.map((r) => [
            r.label,
            r.rows,
            r.durationMs,
          ]);
          downloadCsv(
            `apex-insights-soql-${ts}.csv`,
            ["Query", "Rows", "Duration (ms)"],
            rows,
          );
        } else if (tableType === "dml") {
          const rows = report.data.dml.map((r) => [
            r.operation,
            r.sObject,
            r.rows,
            r.durationMs,
          ...
          downloadCsv(
            `apex-insights-dml-${ts}.csv`,
            ["Operation", "Object", "Rows", "Duration (ms)"],
            rows,
          );
          ...
          downloadCsv(
            `apex-insights-external-calls-${ts}.csv`,
            [
              "Named Credential",
              "Method",
              "Host",
