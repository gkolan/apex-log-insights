...
function isDiagnosticSectionExpanded(state, key) {
  return Array.isArray(state?.expanded) && state.expanded.includes(key);
}

function takeVisibleDiagnosticItems(items, expanded, limit) {
  const arr = Array.isArray(items) ? items : [];
  return expanded ? arr : arr.slice(0, limit);
}

function diagnosticExpandButton(key, expanded, total, limit) {
  if (total <= limit) return "";
  return `<button class="actionBtn" type="button" data-toggle-section="${escapeHtml(key)}" aria-expanded="${expanded ? "true" : "false"}">${expanded ? "Show fewer" : `Show all (${String(total)})`}</button>`;
}

function renderHeapWatermark(summary) {
  const samples = summary?.watermarkSamples?.filter(
    (sample) => sample.timestampNs !== null && sample.cumulativeBytes !== null,
  ) || [];
  if (samples.length < 2) return "";
  const minTime = samples[0].timestampNs;
  const maxTime = samples[samples.length - 1].timestampNs;
  const maxBytes = Math.max(...samples.map((sample) => sample.cumulativeBytes), 1);
  const points = samples.map((sample) => {
    const x = maxTime === minTime ? 0 : ((sample.timestampNs - minTime) / (maxTime - minTime)) * 100;
    const y = 38 - (sample.cumulativeBytes / maxBytes) * 34;
    return `${x.toFixed(2)},${y.toFixed(2)}`;
  }).join(" ");
  return `<figure class="dataFigure compactFigure"><svg class="trajectoryChart" viewBox="0 0 100 42" role="img" aria-labelledby="heapWatermarkTitle heapWatermarkDesc" preserveAspectRatio="none"><title id="heapWatermarkTitle">Heap watermark over the transaction</title><desc id="heapWatermarkDesc">Heap reached ${escapeHtml(formatBytes(maxBytes))} across ${samples.length} samples.</desc><line x1="0" y1="38" x2="100" y2="38" class="chartAxis"></line><polyline points="${points}" class="chartLine chartLineHeap"></polyline></svg><figcaption>${samples.length} samples · peak ${escapeHtml(formatBytes(maxBytes))}</figcaption></figure>`;
}
...
export function renderDiagnostics(report, state = {}) {
  ...
  const showAllIssues = isDiagnosticSectionExpanded(state, "diag-issues");
  const showAllStructural = isDiagnosticSectionExpanded(state, "diag-structural");
  const showAllDebugWarnings = isDiagnosticSectionExpanded(state, "diag-debug-warnings");
  const showAllDebugEvents = isDiagnosticSectionExpanded(state, "diag-debug-events");
  const showAllParserWarnings = isDiagnosticSectionExpanded(state, "diag-parser-warnings");
  const visibleIssues = takeVisibleDiagnosticItems(
    report.diagnostics.issues,
    showAllIssues,
    12,
  );
  const visibleStructural = takeVisibleDiagnosticItems(
    report.diagnostics.structuralWarnings,
    showAllStructural,
    8,
  );
  const visibleDebugWarnings = takeVisibleDiagnosticItems(
    report.diagnostics.debugWarnings,
    showAllDebugWarnings,
    8,
  );
  const visibleDebugEvents = takeVisibleDiagnosticItems(
    report.diagnostics.debugEvents,
    showAllDebugEvents,
    20,
  );
  const visibleParserWarnings = takeVisibleDiagnosticItems(
    report.diagnostics.parserWarnings,
    showAllParserWarnings,
    10,
  );
  return `
    <section class="viewPanel">
      <article class="sectionCard panel">
        ...
        <div class="sectionBody">
          <div class="actionRow" style="margin-bottom:8px;">${diagnosticExpandButton("diag-issues", showAllIssues, report.diagnostics.issueCountTotal || 0, 12)}</div>
          <div class="stack">
            ${
                ...
                <article class="issueRow issue-${escapeHtml(item.severity)}">
                  <div class="issueHead">
                    <div>
                      ...
                      <div class="inlineMeta">${escapeHtml(item.type)}${item.confidence ? ` &middot; ${escapeHtml(item.confidence)}` : ""}</div>
                    </div>
      ...
      <article class="sectionCard panel">
        ...
        <div class="sectionBody">
          <div class="actionRow" style="margin-bottom:8px;">${diagnosticExpandButton("diag-structural", showAllStructural, report.diagnostics.structuralWarnings.length, 8)}</div>
          <div class="stack">
            ${
      ...
      <article class="sectionCard panel">
        ...
        <div class="sectionBody grid3">
          <div class="detailCard">
            ...
            ${
              report.diagnostics.executionContext
                ? `<div class="inlineMeta">${escapeHtml(report.diagnostics.executionContext.label)} &middot; ${escapeHtml(report.diagnostics.executionContext.confidence)}</div>${report.diagnostics.executionContext.signals.length > 0 ? `<div class="smallCopy">${escapeHtml(report.diagnostics.executionContext.signals[0])}</div>` : ""}`
                : '<p class="smallCopy">Execution context not detected.</p>'
            }
          </div>
          <div class="detailCard">
            <strong>Debug Levels</strong>
            <div class="actionRow" style="margin:8px 0;">${diagnosticExpandButton("diag-debug-warnings", showAllDebugWarnings, report.diagnostics.debugWarningCountTotal, 8)}</div>
            ${
          ...
          <div class="detailCard">
            ...
            ${
              heapSummary
                ? `<div class="smallCopy">Peak ${escapeHtml(formatBytes(heapSummary.peakCumulativeBytes))} &middot; Alloc ${escapeHtml(String(heapSummary.allocationCount))} &middot; Free ${escapeHtml(String(heapSummary.deallocationCount))} &middot; Net ${escapeHtml(formatBytes(heapSummary.netAllocatedBytes))}</div>${renderHeapWatermark(heapSummary)}${
                    heapSummary.hotspots && heapSummary.hotspots.length > 0
                      ? `<div class="tableWrap" style="margin-top:8px;"><table><thead><tr><th>Line</th><th>Allocs</th><th>Bytes</th><th>Namespace</th></tr></thead><tbody>${heapSummary.hotspots.map((h) => `<tr><td>${h.line !== null ? escapeHtml(String(h.line)) : "–"}</td><td>${escapeHtml(String(h.allocations))}</td><td>${escapeHtml(formatBytes(h.totalBytes))}</td><td>${escapeHtml(h.namespace || "default")}</td></tr>`).join("")}</tbody></table></div>`
                      : ""
                  }`
              ...
              cpuBreakdown.byNamespace.length === 0
                ...
                <div class="stack" style="margin-top:10px;">
                  ${
                    cpuBreakdown.byType.length === 0
                      ? ""
                      : `<div class="listCard"><strong class="listTitle">By Type</strong><div class="stack">${cpuBreakdown.byType.map((item) => `<div><div class="inlineMeta">${escapeHtml(item.label)} &middot; ${escapeHtml(String(item.durationMs))} ms</div><div class="smallCopy">${escapeHtml(String(item.count))} span(s)</div></div>`).join("")}</div></div>`
                  }
                  ${
                    cpuBreakdown.byNamespace.length === 0
                      ? ""
                      : `<div class="listCard"><strong class="listTitle">By Namespace</strong><div class="stack">${cpuBreakdown.byNamespace.map((item) => `<div><div class="inlineMeta">${escapeHtml(item.namespace)} &middot; ${escapeHtml(String(item.totalDurationMs))} ms total</div><div class="smallCopy">${escapeHtml(String(item.selfDurationMs))} ms self &middot; ${escapeHtml(String(item.spanCount))} span(s)</div></div>`).join("")}</div></div>`
                  }
                </div>
              `
            }
        ...
        <div class="sectionBody" style="padding-top:0;">
          <div class="detailCard">
            ...
            <div class="actionRow" style="margin:8px 0;">${diagnosticExpandButton("diag-parser-warnings", showAllParserWarnings, report.diagnostics.parserWarningCountTotal, 10)}</div>
            ${
      ...
      <article class="sectionCard panel">
        ...
        <div class="sectionBody">
          <div class="actionRow" style="margin-bottom:8px;">${diagnosticExpandButton("diag-debug-events", showAllDebugEvents, report.diagnostics.debugEventCountTotal, 20)}</div>
          ${
