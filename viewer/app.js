...
const DEFAULT_UI_CONFIG = {
  ...
  labels: {
    soqlTopTemplate: "Top {count} Longest-Running Queries (excluding managed packages)",
    soqlExpandedSubheading: "Chronological order (as executed, excluding managed packages)",
    dmlSubheading: "Total records inserted, updated, deleted, or upserted (excluding managed packages)",
...
async function initializeLogExplorerSettings() {
  ...
  if (rawUserDebugOnlyToggle) rawUserDebugOnlyToggle.checked = settings.userDebugOnly;
...
const DEFAULT_RAW_STATUS_HINT = "Hint: Refine with error text + lines:1200-1600";

const resourceUsageBody = document.getElementById("resourceUsageBody");
const toggleResourceUsageBtn = document.getElementById("toggleResourceUsageBtn");
...
const problematicQueriesTable = document.getElementById("problematicQueriesTable");
...
const rawUserDebugOnlyToggle = document.getElementById("rawUserDebugOnlyToggle");
...
const problematicQueriesPanel = document.getElementById("problematicQueriesPanel");
...
const structuralWarningsPanel = document.getElementById("structuralWarningsPanel");
const structuralWarningsList = document.getElementById("structuralWarningsList");
const diagnosticsQualityGrid = document.getElementById("diagnosticsQualityGrid");
const parserWarningsList = document.getElementById("parserWarningsList");
const executionInsightsPanel = document.getElementById("executionInsightsPanel");
...
const executionTypeFilterSelect = document.getElementById("executionTypeFilterSelect");
...
const overviewTabLink = document.getElementById("overviewTabLink");
...
function startLoadingScreenSequence() {
  ...
  const typeStep = (message, charIndex) => {
    if (runId !== loadingScreenRunId || !document.body.classList.contains("initializing")) return;
    setLoadingScreenText(message.slice(0, charIndex));
    if (charIndex < message.length) {
      loadingScreenTimer = setTimeout(() => typeStep(message, charIndex + 1), 34);
      return;
    }
...
loadingStateObserver.observe(document.body, { attributes: true, attributeFilter: ["class"] });
if (document.body.classList.contains("initializing")) {
  startLoadingScreenSequence();
}
...
window.toggleIssuesExpanded = () => { issuesExpanded = !issuesExpanded; if (typeof render === "function" && typeof currentReportData !== "undefined") render(currentReportData, typeof rawLineByTextSource !== "undefined" ? rawLineByTextSource : []); };
let parserWarningsExpanded = false;
window.toggleParserWarningsExpanded = () => { parserWarningsExpanded = !parserWarningsExpanded; if (typeof render === "function" && typeof currentReportData !== "undefined") render(currentReportData, typeof rawLineByTextSource !== "undefined" ? rawLineByTextSource : []); };
...
function syncRawCopyButtons() {
  ...
  if (rawCopyHighlightedBtn) rawCopyHighlightedBtn.hidden = highlighted.length === 0;
  if (rawCopyVisibleBtn) rawCopyVisibleBtn.hidden = visible.length === 0 || allLinesMode || filterMode;
...
async function copyRenderedRawLines(selector, emptyMessage, copiedLabel, button) {
...
function shouldForceClearButtonForQuery(query) {
  return isAllLinesQuery(query) || isSingleLineQuery(query) || isRangeLineQuery(query);
}
...
function normalizeRawHint(hint) {
  ...
  return text
    .replace(/^Tip:/i, "Hint:")
    .replace(/^Try Filter:/i, "Refine:");
}

function setRawStatus(message, hint) {
  if (!rawSearchStatus) return;
  rawSearchStatus.textContent = String(message || "");
  const resolvedHint = hint ?? buildRawHintForQuery(rawSearchInput?.value || "", getSelectedContextRows());
  if (rawUsageHint) {
...
function parseScopedTextRangeQuery(query) {
  ...
  if (!term || !Number.isFinite(rangeStart) || !Number.isFinite(rangeEnd)) return null;
  return {
    term,
...
function syncRawControlsFromQuery(query) {
  ...
  if (hasQuery) {
    ...
    if (rawContextSelect) {
      const isFiltered = isUserDebugOnlyMode() || isErrorsOnlyMode() || Boolean(rawShowAllToggle?.checked);
...
function setExpandButtonLabel(button, expanded, count) {
  ...
  button.textContent = fillCountTemplate(uiConfig.buttons.showMoreTemplate, count);
}
...
function getReportExecution(report) {
  ...
  return {
    blocks: Array.isArray(execution?.blocks)
      ? execution.blocks
      : [],
    tree: Array.isArray(execution?.tree)
      ? execution.tree
      : [],
    trace: report?.trace || {},
  };
...
function getReportIssues(report) {
  ...
  const analysisWarnings = [
    ...(Array.isArray(report?.analysis?.warnings) ? report.analysis.warnings : []),
    ...(Array.isArray(report?.analysis?.phaseWarnings) ? report.analysis.phaseWarnings : []),
  ].map((item, index) => ({
    id: item?.id || `analysis-warning-${index + 1}`,
    type: "warning",
    summary: item?.title || item?.phase || item?.summary || item?.message || "Warning",
...
function getReportGovernorLimits(report) {
  ...
  const snapshots = Array.isArray(canonical?.snapshots) ? canonical.snapshots : [];
  return {
    current,
    snapshots,
  };
...
function hasRecoverableSourceInUrl() {
  try {
    ...
    return Boolean(String(params.get("sourceUrl") || "").trim() || String(params.get("storageKey") || "").trim());
  } catch {
    return false;
  }
}
...
async function createNewTabHandoffUrl(href) {
  ...
  const fileSizeBytes = rawLogLines.length > 0
...
function issueTable(kindClass, rows) {
  ...
  const body = rows
    .map((r) => {
      ...
      const lineDisplay = uniqueLineNumbers.length > 1
        ? `<div class="issueLineList">${uniqueLineNumbers.map((n) => `<a href="${buildRawLogHref(`line:${n}`)}" class="jumpRawFromQuery" data-line="${n}">${n}</a>`).join("")}</div>`
        : lineQuery
        ...
        : (lineNum !== null
          ? `<a href="${buildRawLogHref(`line:${lineNum}`)}" class="jumpRawFromQuery" data-line="${lineNum}">${lineValue}</a>`
          : "-");
...
function renderGovernorLimits(target, report) {
  ...
  const nsContext = (context?.org?.namespaceContext || []).filter((ns) => ns && ns !== "default");
  ...
  const hiddenManagedNamespaces = nsContext.filter((ns) => !shownManagedNamespaces.has(ns));
  ...
  const defaultCards = defaultEntries
    .map(
      (entry) => {
        ...
        const pctDisplay = Number(entry.max || 0) > 0 ? `${Math.round(pct)}%` : "";
        return `
      <div class="kv limitCard">
        ...
        ${Number(entry.max || 0) > 0 ? `<div class="limitBar"><div class="limitBarFill ${barClass}" style="width:${pct}%"></div></div><span class="limitPercent">${pctDisplay}</span>` : ""}
      </div>`;
      },
    )
    .join("");
  ...
  const managedRows = managedGroups
    .map((group) => {
      const metrics = group.entries
        .map((entry) => `<span class="govMetric">${governorLimitLabel(entry.key)}: ${governorLimitValue(entry.key, entry.used, entry.max)}</span>`)
        .join(" ");
      return `<div class="govNsRow"><span class="govNsName">${escapeHtml(group.namespace)}</span><span class="govNsMetrics">${metrics}</span></div>`;
    })
    .join("");

  const hiddenText = hiddenManagedNamespaces.length > 0
...
function normalizeObjectApiName(name) {
  ...
  if (!text) return text;
  if (text.endsWith("__c") || text.endsWith("__e") || text.endsWith("__mdt")) return text;
...
function compactLabel(label) {
  const text = String(label || "");
  const triggerMatch = text.match(/^(.+?) on (.+?) trigger event (Before|After)(Insert|Update|Delete|Undelete)$/i);
  if (triggerMatch) {
  ...
  if (text.startsWith("Validation:")) {
    ...
    if (validation.context && !validation.recordId && !/^new$/i.test(validation.context)) {
  ...
  if (text.startsWith("Workflow:")) return text.replace("Workflow:", "Workflow · ");
  return text;
}
...
function flowNameFromBlock(flowBlock) {
  const fromStart = (flowBlock?.steps || []).find((step) =>
    String(step?.type || "") === "FLOW_START_INTERVIEW_BEGIN",
  );
  ...
  if (fromStart?.text) {
    ...
    if (fallbackName && !/^FLOW_START_INTERVIEW_BEGIN$/i.test(fallbackName)) return fallbackName;
  }
  ...
  return String(flowBlock?.label || "").replace(/^Flow:/, "").trim() || "-";
}
...
function resolveFlowBlockForDisplay(report, block) {
  ...
  const siblingCandidates = spans
    ...
    .map((s) => {
      const siblingFlowBlock = flowBlocks.find((fb) => fb?.eventId === s?.id) || null;
...
function queryLineLink(rawLineNumber, fallbackLineNumber) {
  const line = Number(rawLineNumber ?? fallbackLineNumber);
  if (!Number.isFinite(line)) return "-";
  return `<a href="${buildRawLogHref(`line:${line}`)}" class="jumpRawFromQuery" data-line="${line}">${line}</a>`;
}
...
function isHiddenInternalExecutionRow(row) {
  ...
  if (!["METHOD_ENTRY", "SYSTEM_METHOD_ENTRY"].includes(eventType)) return false;
...
function getExecutionEventCategory(eventType) {
  ...
  if (FLOW_EVENT_TYPES.has(type) || type.startsWith("FLOW_") || type.startsWith("WF_FLOW_ACTION_")) return "flow";
  if (type.startsWith("VALIDATION_") || type === "VALIDATION_RULE") return "validation";
  if (type.startsWith("SOQL_")) return "soql";
  if (type.startsWith("DML_")) return "dml";
  if (type.startsWith("WORKFLOW_") || type.startsWith("WF_") || type.startsWith("PROCESS_")) return "workflow";
  if (type === "EXCEPTION_THROWN" || type === "FATAL_ERROR" || type === "VF_FATAL_ERROR") return "exception";
  if (type === "USER_DEBUG") return "debug";
  if (type.startsWith("CODE_UNIT_")
...
function getExecutionEventCategoryLabel(categoryKey) {
  return EXECUTION_EVENT_CATEGORY_LABELS[categoryKey] || EXECUTION_EVENT_CATEGORY_LABELS.other;
}
...
function isExecutionStoryEventType(eventType) {
  ...
  if (FLOW_EVENT_TYPES.has(type) || type.startsWith("FLOW_") || type.startsWith("WF_FLOW_ACTION_")) return true;
  if (type.startsWith("VALIDATION_")) return true;
  if (type.startsWith("SOQL_") || type.startsWith("SOSL_") || type.startsWith("DML_")) return true;
  if (type.startsWith("WORKFLOW_") || type.startsWith("WF_") || type.startsWith("PROCESS_")) return true;
  if (type.startsWith("METHOD_") || type.startsWith("CONSTRUCTOR_")) return true;
  if (type.startsWith("QUEUEABLE_") || type.startsWith("FUTURE_") || type.startsWith("BATCH_")) return true;
  if (type.startsWith("SYSTEM_METHOD_") || type.startsWith("SYSTEM_CONSTRUCTOR_")) return true;
  if (type.startsWith("TRIGGER_")) return true;
  if (type.startsWith("CUMULATIVE_") || type.startsWith("LIMIT_USAGE")) return true;
  return false;
}
...
function syncExecutionTypeFilterUi(categoryKeys) {
  ...
  const options = EXECUTION_EVENT_CATEGORY_ORDER.filter((key) =>
    key === EXECUTION_EVENT_CATEGORY_ALL || available.has(key),
  );
  setHtml(executionTypeFilterSelect, options
    .map((key) => `<option value="${escapeHtml(key)}">${escapeHtml(getExecutionEventCategoryLabel(key))}</option>`)
    .join(""));
  executionTypeFilterSelect.value = options.includes(selectedExecutionEventCategory)
    ? selectedExecutionEventCategory
    : EXECUTION_EVENT_CATEGORY_ALL;
  if (!options.includes(executionTypeFilterSelect.value) && options.length > 0) {
...
function renderCardGrid(target, cards, emptyMessage, options = {}) {
  if (!target) return;
  target.classList.toggle("reportCardGrid--clustered", Boolean(options.clustered));
  const rows = (cards || []).filter(Boolean);
  if (rows.length === 0) {
    setHtml(target, `<div class="reportCardsEmpty">${escapeHtml(emptyMessage || "No data.")}</div>`);
    return;
  }
    ...
    setHtml(target, `
      <div class="reportMetricCluster">
        ${rows.map((card) => {
          ...
          const tagAttrs = card.href ? ` href="${escapeHtml(card.href)}"` : "";
...
function formatScopeGroupLabel(label) {
  ...
  const formatted = base
    ...
    .map((part) => {
      ...
      if (/^[a-z0-9]+$/.test(part)) return part.charAt(0).toUpperCase() + part.slice(1);
      return part;
    })
    .join("_");
...
function buildScopeIdGroups(report, scopeIds) {
  const recordGraph = Array.isArray(report?.recordGraph) ? report.recordGraph : [];
  ...
  for (const id of scopeIds) {
    ...
    const rawLabel = labelByPrefix.get(prefix) || STANDARD_SCOPE_PREFIX_LABELS[prefix] || `Unknown (${prefix || "n/a"})`;
...
function getScopeGroupReportKey(report) {
  ...
  const fileName = String(input?.fileName || source?.fileName || currentReportUrl || "").trim();
  const generatedAt = String(source?.generatedAt || input?.generatedAt || report?.generatedAt || "").trim();
...
function renderScopeIdGroups(groups) {
  if (!groups.length) return '<div class="scopeGroupEmpty">No scope IDs available</div>';
  return `
    <div class="scopeGroupList">
      ${groups.map((group) => {
        ...
        const renderChildLabel = (childGroup) => {
          ...
          const prefix = String(childGroup.prefix || "").toUpperCase() || "N/A";
          ...
          <div class="scopeGroupRow">
            ...
            ${expanded ? `
              <div class="scopeGroupIds">
                ${Array.isArray(group.childGroups) && group.childGroups.length > 0
...
function extractIssueSearchTerm(item) {
  ...
  const typedMapMatch = summary.match(/^Map<[^>]+>\s+([A-Za-z0-9_]+)\s+is empty$/i);
...
function issueIcon(item) {
  ...
  if (type.includes("TRIGGER") || type.includes("RECURSION") || type.includes("RECURSIVE")) {
    // Lucide RotateCcw
...
function issueEvidenceLines(item, rawLineForEvidence) {
  ...
  const values = Array.from(new Set([
    ...fromArray,
    ...(Number.isFinite(fromSingleRaw) && fromSingleRaw >= 1 ? [fromSingleRaw] : []),
...
function isSpecificEmptyResultMapWarning(item) {
  ...
  return (
    type === "EMPTY_RESULT_MAP_WARNING"
    && /(?:^Map<[^>]+>\s+\S+\s+is empty$)|(?:^<Map>\s+\S+\s+is empty$)/i.test(summary)
  );
}
...
function synthesizeEmptyMapWarningsFromRawLog(rawLogLines, existingIssues) {
  ...
  const hasSpecific = (existingIssues || []).some((item) => isSpecificEmptyResultMapWarning(item));
  const hasAggregate = (existingIssues || []).some((item) => isAggregateEmptyResultMapWarning(item));
  if (hasSpecific || !hasAggregate) return [];

  const mapTypeByName = new Map();
  for (const line of rawLogLines) {
    const scopeMatch = String(line || "").match(/\|VARIABLE_SCOPE_BEGIN\|\[\d+\]\|([^|]+)\|(Map<[^|>]+(?:>[^|]*)?>)\|/);
  ...
  for (let i = 0; i < rawLogLines.length; i += 1) {
    ...
    const assignMatch = raw.match(/\|VARIABLE_ASSIGNMENT\|\[\d+\]\|([^|]+)\|\{\}\|/);
    if (!assignMatch) continue;
    const variableName = (assignMatch[1] || "").trim();
    const variableType = mapTypeByName.get(variableName) || "";
    const looksLikeMap = /^map/i.test(variableName) || /^Map</.test(variableType);
...
function parseUserInfoFromRawLog(lines) {
  const row = (lines || []).find((line) => String(line || "").includes("|USER_INFO|"));
  if (!row) {
...
function normalizeRawLogEntry(value) {
  return String(value ?? "").replace(/\r$/, "").trimEnd();
}
...
function isUserDebugEventType(eventType) {
  ...
  return type === "USER_DEBUG" || type === "DATAWEAVE_USER_DEBUG" || type.startsWith("USER_DEBUG_");
}
...
function parseJsonWithDiagnostics(text) {
  ...
  const fail = (message) => ({ ok: false, error: { message, index }, duplicateWarnings, numericWarnings });
  const isDigit = (ch) => ch >= "0" && ch <= "9";
  const tick = () => {
    parseSteps += 1;
    if (parseSteps > PRETTY_MAX_JSON_PARSE_STEPS) {
      return fail(`Parse step limit exceeded (${PRETTY_MAX_JSON_PARSE_STEPS.toLocaleString()}).`);
    }
    return null;
  };
  const skipWs = () => {
  ...
  const parseString = () => {
    if (source[index] !== "\"") return fail('Expected string opening quote (").');
    index += 1;
    let out = "";
    while (index < source.length) {
      ...
      if (ch === "\\") {
        index += 1;
        if (index >= source.length) return fail("Unterminated escape sequence.");
        ...
        if (esc === "u") {
          ...
          if (!/^[0-9a-fA-F]{4}$/.test(hex)) return fail("Invalid unicode escape sequence.");
          out += " ";
          index += 5;
          continue;
        }
    ...
    if (/^-?\d+$/.test(token)) {
      try {
        ...
        if (n > MAX_SAFE || n < -MAX_SAFE) {
          numericWarnings.push(`${path} has an integer outside JS safe precision range.`);
        }
      } catch {
  ...
  const parseLiteral = (literal) => {
    if (source.slice(index, index + literal.length) !== literal) return fail(`Expected "${literal}".`);
...
function extractJsonCandidatesFromRawLine(rawLine) {
  ...
  if (eventType === "VARIABLE_ASSIGNMENT" || eventType === "VARIABLE_SCOPE_BEGIN") {
    if (parts.length < 5) return [];
    const lastPart = String(parts[parts.length - 1] || "").trim();
    const hasHash = /^0x[0-9a-fA-F]+$/.test(lastPart);
    const value = (hasHash ? parts.slice(4, -1) : parts.slice(4)).join("|").trim();
    if (looksLikeJsonLiteral(value)) {
      ...
      for (const span of findJsonSpans(value, 2)) {
        candidates.push({ literal: span, source: "variable-embedded", eventType });
      }
    }
    return candidates;
  }
  ...
  if (isUserDebugEventType(eventType) && parts.length >= 5) {
    ...
    if (looksLikeJsonLiteral(message)) {
      ...
      for (const span of spans) candidates.push({ literal: span, source: "user-debug-embedded", eventType });
      const escapedSpans = extractEscapedJsonLiterals(message, 3);
      for (const span of escapedSpans) candidates.push({ literal: span, source: "user-debug-escaped-json", eventType });
    }
    return candidates;
  }
...
function analyzeJsonCandidate(candidate) {
  ...
  if (!literal) return { ok: false, status: "error", reason: "No JSON literal detected.", text: null, warnings: [] };
  const warnings = [];
  if (/[\u202A-\u202E\u2066-\u2069\u200B-\u200F]/.test(literal)) {
    warnings.push("Contains invisible/bidirectional Unicode control characters.");
  }
  ...
  return { ok: true, status: "formatted", reason: null, text: pretty, warnings };
}

function formatJsonPretty(rawLine) {
  const candidates = extractJsonCandidatesFromRawLine(rawLine);
  if (candidates.length === 0) {
    return { status: "error", reason: "No JSON literal detected.", text: null, warnings: [] };
  }
  const analyses = candidates.map((candidate) => analyzeJsonCandidate(candidate));
  const successes = analyses.filter((analysis) => analysis.ok);
  if (successes.length > 0) {
    const combinedWarnings = successes.flatMap((item) => item.warnings || []).slice(0, 12);
    const formattedDocs = successes
      .map((item, idx) => (successes.length > 1 ? `# JSON ${idx + 1}\n${item.text || ""}` : (item.text || "")))
      .filter(Boolean);
    const combinedText = formattedDocs.join("\n\n---\n\n");
    const status = successes.some((item) => item.status === "truncated") ? "truncated" : "formatted";
    return {
      status,
      reason: successes.length > 1 ? `Detected ${successes.length} JSON payloads in one line.` : successes[0]?.reason || null,
      text: combinedText,
...
function extractSoqlResultFromRawLine(rawLine) {
  ...
  return parts.slice(3).map((p) => String(p || "").trim()).filter(Boolean);
}
...
function extractDmlFromRawLine(rawLine) {
  ...
  return {
    ...
    fields: parts.slice(3).map((p) => String(p || "").trim()).filter(Boolean),
  };
}
...
function extractIntegrationFromRawLine(rawLine) {
  ...
  return { eventType, body, fields: parts.slice(3).map((p) => String(p || "").trim()).filter(Boolean) };
}
...
function extractApexSpanFromLine(rawLine) {
  ...
  if (!isUserDebugEventType(eventType) && !allowedTypes.includes(eventType)) return null;
  ...
  if (typedWrapper && typedWrapper.index !== undefined) {
    ...
    start = [braceIdx, parenIdx, bracketIdx]
      .filter((idx) => idx >= 0)
      .sort((a, b) => a - b)[0] ?? -1;
  }
...
function formatSoqlCore(normalized, indent) {
  ...
  const formattedFields = fields
    .map((f) => {
      ...
      return /^\(SELECT\s/i.test(trimmed) ? formatSoqlSubquery(trimmed, fieldIndent) : fieldIndent + trimmed;
    })
    .join(",\n");
...
function formatIntegrationPretty(rawLine) {
  ...
  const statusMatch = text.match(/\b(?:status(?:Code)?|http)\s*[:=]?\s*(\d{3})\b/i);
...
function getPrettyPayloadForRawLine(rawLine) {
  ...
  if (kind === "json") {
    ...
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "SOQL parse unavailable.", warnings: [], text };
  } else if (kind === "sosl") {
    const text = formatSoslPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "SOSL parse unavailable.", warnings: [], text };
  } else if (kind === "sosl-result") {
    const text = formatSoslResultPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "SOSL result details unavailable.", warnings: [], text };
  } else if (kind === "soql-explain") {
    const text = formatSoqlExplainPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "SOQL explain details unavailable.", warnings: [], text };
  } else if (kind === "soql-result") {
    const text = formatSoqlResultPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "SOQL result details unavailable.", warnings: [], text };
  } else if (kind === "dml") {
    const text = formatDmlPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "DML details unavailable.", warnings: [], text };
  } else if (kind === "integration") {
    const text = formatIntegrationPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "Integration details unavailable.", warnings: [], text };
  } else if (kind === "exception") {
    const text = formatExceptionPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "Exception details unavailable.", warnings: [], text };
  } else if (kind === "apex") {
    const text = formatApexPretty(rawLine);
    payload = { kind, status: text ? "formatted" : "error", reason: text ? null : "Apex object parse unavailable.", warnings: [], text };
  } else if (kind === "range") {
    payload = { kind, status: "formatted", reason: null, warnings: [], text: null };
  }
  ...
  if (payload && payload.text && typeof payload.text === "string") {
    ...
    if (squashed === "{}" || squashed === "[]" || squashed === "()" || squashed === "") {
      payload = null;
    }
  }
...
function rawLineHtml(lineNumber, lineText, cls) {
  ...
  } else if (lineText.includes("|METHOD_ENTRY|") || lineText.includes("|CONSTRUCTOR_ENTRY|")) {
  ...
  const prettyLabel =
    kind === "json" ? "Pretty JSON" :
    kind === "soql" ? "Pretty SOQL" :
    kind === "sosl" ? "Pretty SOSL" :
    kind === "sosl-result" ? "SOSL Result Summary" :
    kind === "soql-explain" ? "Pretty SOQL Explain" :
    kind === "soql-result" ? "SOQL Result Summary" :
    kind === "dml" ? "DML Summary" :
    kind === "integration" ? "Integration Summary" :
    kind === "exception" ? "Exception Summary" :
    kind === "apex" ? "Pretty Apex" :
    kind === "range" ? "Show log line range" : "";
  let rangeAttr = "";
  if (kind === "range") {
  ...
  if (isExpanded) {
    ...
    if (kind === "range") {
      ...
      if (from > 0 && to >= from) {
        ...
        for (let i = from; i <= hardEnd; i += 1) lines.push(`${i}: ${currentRawLogLines[i - 1] || ""}`);
        if (hardEnd < to) {
          lines.push("");
          lines.push(`... [truncated range: showing ${PRETTY_MAX_RANGE_LINES.toLocaleString()} of ${(to - from + 1).toLocaleString()} lines]`);
        }
  ...
  const prettyControls = kind
    ...
    : "";
  return `<div class="${finalCls}" data-raw-line-number="${lineNumber}"><span class="rawLineNo">${lineNumber}</span><span class="rawLineText">${escapeHtml(lineText)}</span>${prettyControls}</div>`;
}
...
function _renderRawLogSearchCore(query) {
  ...
  if (!currentRawLogLines.length) {
    setRawStatus("Raw log file not found for this report.", "Hint: Load a .log file, then run search.");
    setHtml(rawLogText, "");
    setRawClearButtonVisibility(false);
    return;
  }

  const renderRawLineRange = (startLine, endLine, statusBuilder, exactMatches = new Set()) => {
    ...
    for (let lineNumber = start; lineNumber <= end; lineNumber += 1) {
      ...
      const cls = isExact ? "rawLine rawLineMatch" : (isErr ? "rawLine rawLineError" : "rawLine");
      rows.push(rawLineHtml(lineNumber, lineText, cls));
    }
    if (rows.length === 0) {
      setRawStatus(userDebugOnly
        ? `No ${USER_DEBUG_LABEL.toLowerCase()} lines in this range.`
        : (errorsOnly ? "No error lines in this range." : "No lines in this range."));
      setHtml(rawLogText, "<p>No matches.</p>");
      setRawClearButtonVisibility(forceClear);
      return;
    }
    setRawStatus(typeof statusBuilder === "function"
  ...
  if (!q) {
    const rows = [];
    const maxRows = (userDebugOnly || errorsOnly || showAllFromToggle) ? Number.POSITIVE_INFINITY : 100;
    ...
    setRawStatus(userDebugOnly
      ...
      : (errorsOnly
        ...
        : (showAllFromToggle
          ...
          : `Showing first ${rows.length} lines.`)));
    setHtml(
      rawLogText,
      rows.length > 0 ? rows.join("") :
        (userDebugOnly ? `<p>No ${USER_DEBUG_LABEL.toLowerCase()} lines found.</p>` :
          errorsOnly ? "<p>No error lines found.</p>" : "<p>No lines.</p>"),
    );
    setRawClearButtonVisibility(false);
    return;
  }
  ...
  if (allSyntax) {
    renderRawLineRange(1, currentRawLogLines.length, () =>
      userDebugOnly
        ...
        : (errorsOnly ? "Showing all error lines." : `Showing all ${currentRawLogLines.length} lines.`),
    );
    return;
  }
  ...
  if (scopedRangeQuery) {
    const start = Math.max(1, Math.min(currentRawLogLines.length, scopedRangeQuery.start));
    const end = Math.max(start, Math.min(currentRawLogLines.length, scopedRangeQuery.end));
    ...
    for (let lineNumber = start; lineNumber <= end; lineNumber += 1) {
      ...
      const isErr = errorsOnly && isErrorLine(lineText);
      const cls = isMatch ? "rawLine rawLineMatch" : (isErr ? "rawLine rawLineError" : "rawLine rawLineContext");
      rows.push(rawLineHtml(lineNumber, lineText, cls));
    }
    if (rows.length === 0) {
      setRawStatus(userDebugOnly
        ? `No ${USER_DEBUG_LABEL.toLowerCase()} lines in lines ${start}-${end}.`
        : (errorsOnly ? `No error lines in lines ${start}-${end}.` : `No lines in lines ${start}-${end}.`));
      setHtml(rawLogText, "<p>No matches.</p>");
  ...
  const singleLineSyntax = q.match(/^lines?\s*:\s*(\d+)\s*$/i);
  if (singleLineSyntax) {
    ...
    renderRawLineRange(start, end, () => contextRows > 0 ? `Showing line ${line} with +/-${contextRows} lines.` : `Showing line ${line}.`, new Set([line]));
    return;
  }
  ...
  if (rangeSyntax) {
    ...
    renderRawLineRange(start, end, () => contextRows > 0 ? `Showing lines ${startObj}-${endObj} with +/-${contextRows} lines.` : `Showing lines ${startObj}-${endObj}.`, exactMatches);
    return;
  }
  ...
  if (lineListSyntax) {
    ...
    for (let i = 0; i < merged.length; i += 1) {
      ...
      for (let lineNumber = range.start; lineNumber <= range.end; lineNumber += 1) {
        ...
        const isExact = exactMatches.has(lineNumber);
        const isErr = errorsOnly && isErrorLine(lineText);
        const cls = isExact ? (isErr ? "rawLine rawLineMatch rawLineError" : "rawLine rawLineMatch") : (isErr ? "rawLine rawLineError" : "rawLine rawLineContext");
        rows.push(rawLineHtml(lineNumber, lineText, cls));
      }
    ...
    if (rows.length === 0) {
      setRawStatus(userDebugOnly
        ...
        : (errorsOnly ? "No error lines matched this query." : "No matches."));
      setHtml(rawLogText, "<p>No matches.</p>");
      setRawClearButtonVisibility(forceClear);
      return;
    }
    const debugLabel = userDebugOnly ? `${USER_DEBUG_LABEL} ` : (errorsOnly ? "error " : "");
    setRawStatus(contextRows > 0 ? `Showing ${debugLabel}lines ${uniqueLines.join(", ")} with +/-${contextRows} lines.` : `Showing ${debugLabel}lines ${uniqueLines.join(", ")}.`);
    setHtml(rawLogText, rows.join(""));
    setRawClearButtonVisibility(forceClear);
    return;
  }
  const qLower = q.toLowerCase();
  const lineSyntax = q.match(/^line\s*:\s*(\d+)$/i);
  ...
  for (let i = 0; i < currentRawLogLines.length; i += 1) {
    ...
    if (userDebugOnly && !isUserDebugLine(lineText)) continue;
    if (errorsOnly && !isErrorLine(lineText)) continue;
    const textMatch = exactLineOnly ? false : lineText.toLowerCase().includes(qLower);
    const lineMatch = qLine !== null
      ? lineNumber === qLine
      : String(lineNumber).includes(q);
  ...
  for (let i = 0; i < merged.length; i += 1) {
    ...
    for (let lineNumber = range.start; lineNumber <= range.end; lineNumber += 1) {
      ...
      const isErr = errorsOnly && isErrorLine(lineText);
      let cls;
      if (isMatch) cls = isErr ? "rawLine rawLineMatch rawLineError" : "rawLine rawLineMatch";
  ...
  if (rows.length === 0) {
    setRawStatus(userDebugOnly
      ? `No ${USER_DEBUG_LABEL.toLowerCase()} lines matched this query.`
      : (errorsOnly ? "No error lines matched this query." : `No matches for "${q}".`));
    setHtml(rawLogText, "<p>No matches.</p>");
    setRawClearButtonVisibility(forceClear);
    return;
  }
...
function setViewModeFromHash() {
  ...
  overviewTabLink?.classList.toggle("activeTab", overviewMode);
...
function renderOverviewPanel(target, report) {
  ...
  const durationMs = derivedDurationMs ||
    (Number.isFinite(Number(context?.transaction?.durationMs)) && Number(context?.transaction?.durationMs) > 0
      ? Number(context?.transaction?.durationMs) : null);
  const cpuMs = Number(context?.transaction?.cpuTimeMs) || null;

  const entryName = entryPoint?.name || "-";
  const entryType = String(entryPoint?.type || context?.transaction?.requestType || "Unknown")
    .replace(/[_-]+/g, " ")
    .replace(/\b\w/g, (c) => c.toUpperCase())
    .trim();
  const user = context?.user?.username || context?.user?.userId || "-";
  const startedAt = String(context?.transaction?.startTimestamp || "").replace("T", " ").trim() || "-";

  const soql = database.soql;
  const dml = database.dml;
  const totalSoqlRows = soql.reduce((sum, q) => sum + Number(q.rows || 0), 0);
  const totalDmlRows = dml.reduce((sum, d) => sum + Number(d.rows || 0), 0);
  const topSoql = soql.slice().sort((a, b) => Number(b.durationMs || 0) - Number(a.durationMs || 0)).slice(0, 10);
  const topDml = dml.slice(0, 10);

  const soqlTableMarkup = topSoql.length === 0 ? "" : `
    <div class="rawLogHeader" style="margin-top:24px;">
      <div>
        <h2>SOQL Queries</h2>
        <p class="subheading sectionSubheading">Top ${topSoql.length} longest-running${soql.length > 10 ? ` of ${soql.length}` : ""}</p>
      </div>
    </div>
    <div class="tableWrap">
      <table class="queryTable">
        ...
        <tbody>
          ${topSoql.map((q) => `
            <tr>
              <td class="mono smallCopy">${escapeHtml(String(q.query || "-").replace(/\s+/g, " ").trim().slice(0, 150))}</td>
  ...
  const dmlTableMarkup = topDml.length === 0 ? "" : `
    <div class="rawLogHeader" style="margin-top:24px;">
      <div>
        <h2>DML Operations</h2>
        <p class="subheading sectionSubheading">${dml.length} total</p>
      </div>
    </div>
    <div class="tableWrap">
      <table>
  ...
  setHtml(target, `
    ...
    <div class="overviewSummaryGrid">
      ...
      <div class="detailCard">
        ...
        <div>${soql.length} queries / ${totalSoqlRows} rows</div>
        <p class="smallCopy">DML</p>
        <div>${dml.length} statements / ${totalDmlRows} rows</div>
...
function render(report, rawLogLines = []) {
  ...
  const rawLineForEvidence = (evidenceRaw) => {
  ...
  const traceEvents = Array.isArray(execution?.trace?.events) ? execution.trace.events : [];
  const codeUnitsAll = traceEvents
    .filter((event) => toScreamingSnakeCase(event?.type || "") === "CODE_UNIT_STARTED")
    .sort((a, b) => {
  ...
  const defaultLimits = governorLimits?.current?.defaultNamespace || {};
  const executionTypeLabel = String(entryPoint?.type || context?.transaction?.requestType || "")
    .replace(/[_-]+/g, " ")
  ...
  const startedAtLabel = (() => {
    ...
    const normalized = value.replace("T", " ").replace(/\s*(Z|[+-]\d{2}:?\d{2})$/, "").trim();
    const match = normalized.match(/(\d{4}-\d{2}-\d{2}|\d{1,2}[/-]\d{1,2}[/-]\d{4}).*?(\d{2}:\d{2}:\d{2}(?:\.\d{1,3})?)/);
    if (match) return `${match[1]} ${match[2]}`;
    if (reportDateFallback && /^\d{2}:\d{2}:\d{2}(?:\.\d{1,3})?$/.test(normalized)) {
      return `${reportDateFallback} ${normalized}`;
    }
    return normalized || value;
  })();

  const soqlRows = defaultLimits?.soqlRows?.used ?? report?.overview?.topMetrics?.soql?.rows;
  const dmlRows = defaultLimits?.dmlRows?.used ?? report?.overview?.topMetrics?.dml?.rows;
  ...
  const rootClassName = entryPoint?.name || codeUnitLabelFromEvent(rootBlock) || null;
  if (topbarClassName) {
    ...
    const triggerOnMatch = displayName.match(/^(\S+)\s+on\s+\S+\s+trigger\s+event\b/i);
    if (triggerOnMatch) {
  ...
  const fileName = report?.source?.fileName || report?.source?.input?.fileName || null;
  ...
  if (topBannerStats) {
    ...
    if (durationMs !== null) {
      const runtimeLabel = durationMs < 1000
  ...
  const syntheticEmptyMapWarnings = synthesizeEmptyMapWarningsFromRawLog(rawLogLines, parserIssues);
  const issues = [...parserIssues, ...syntheticEmptyMapWarnings]
    .filter((item) => {
      if (!isAggregateEmptyResultMapWarning(item)) return true;
      const hasDetailedMapWarnings = [...parserIssues, ...syntheticEmptyMapWarnings]
  ...
  const hasIssueEvidence = (item) => {
    ...
    return rawLineForEvidence(item?.evidence?.raw) !== null;
  };
  const evidenceBackedIssues = issues
    ...
    .map((item) => {
      ...
      const resolved = rawLineForEvidence(ev.raw);
  ...
  const namespaces = Array.from(new Set([
    ...
    ...evidenceBackedIssues.map((item) => String(item?.namespace || "").trim()),
  ].filter(Boolean)));
  const managedNamespaces = namespaces.filter((ns) => {
    const normalized = String(ns || "").trim().toLowerCase();
    return normalized && normalized !== "default";
  });
  const scopeIds = Array.from(new Set((entryPoint?.recordIds || []).map((id) => String(id || "").trim()).filter(Boolean)));
  ...
  if (scopeIdGroups.length > 0 && expandedScopeGroupKeys.size === 0 && !scopeGroupUserHasToggled) {
    expandedScopeGroupKeys.add(scopeIdGroups[0].key);
  }
  const issueRows = (items) => (issuesExpanded ? items : items.slice(0, 200)).map((item) => ({
    nameRaw: String(item?.summary || item?.type || "-"),
    name: escapeHtml(item?.summary || item?.type || "-"),
    copyValue: extractIssueSearchTerm(item),
    type: escapeHtml(prettifyIssueType(item?.type || (isWarningItem(item) ? "Warning" : "Exception"))),
    rawLine: rawLineForEvidence(item?.evidence?.raw),
    lineQuery: (() => {
  ...
  const mockIssueCount = Math.max(1, Number(uiConfig.mock.issueMockCount || 10));
  const errorRows = errors.length > 0 ? issueRows(errors) : (useMockIssues ? buildMockIssueRows("error", mockIssueCount) : []);
  const warningRows = warnings.length > 0 ? issueRows(warnings) : (useMockIssues ? buildMockIssueRows("warning", mockIssueCount) : []);
  const totalIssueCount = errorRows.length + warningRows.length;

  if (issuesPanel) issuesPanel.hidden = totalIssueCount === 0;
  if (issuesSubheading) {
    issuesSubheading.textContent = totalIssueCount === 0
      ? ""
  ...
  if (issuesTable) {
    if (totalIssueCount === 0) {
      ...
      let html = issueTable("issueError", errorRows) + issueTable("issueWarn", warningRows);
      ...
      issuesTable.querySelector("[data-toggle-issues-expanded]")?.addEventListener("click", () => window.toggleIssuesExpanded());
    }
    promoteCopyCellsToTd(issuesTable);
  }

  renderCardGrid(triageSnapshotGrid, [
    {
      title: "Outcome",
    ...
    },
    {
      ...
      icon: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2-2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/></svg>`,
      value: managedNamespaces.length > 0
    ...
    },
    {
      ...
      value: renderMetricPairs([
        [pluralizeLabel(database.soql.length, "SOQL Query", "SOQL Queries"), pluralizeLabel(soqlRows, "Row Read", "Rows Read")],
        [pluralizeLabel(database.dml.length, "DML Statement", "DML Statements"), pluralizeLabel(dmlRows, "Row Written", "Rows Written")],
    ...
    },
    {
      ...
      detailHtml: scopeIdGroups.length > 0 ? renderScopeIdGroups(scopeIdGroups) : "",
      span: "full",
    },
  ], "No triage snapshot data.", { clustered: true });
  ...
  if (typeof buildVerdict === "function" && typeof verdictPanel !== "undefined" && verdictPanel) {
    ...
    const verdictIcons = { ok: "✓", info: "ℹ", warn: "⚠", critical: "⚠", error: "✕" };
    const icon = verdictIcons[verdictData.level] || "ℹ";
    setHtml(verdictPanel, `<div class="verdict-card verdict-card--${escapeHtml(verdictData.level)}"><span class="verdict-icon">${icon}</span><span class="verdict-text">${escapeHtml(verdictData.text)}</span></div>`);
  ...
  renderRichList(
    ...
    evidenceBackedIssues.slice(0, 5).map((item) => {
      ...
      const lineText = lineNumbers.length > 0
        ? `<span class="findingLineInline">at ${lineNumbers.length === 1 ? "line" : "lines"} ${lineNumbers.map((line) => `<a class="reportEvidenceLink jumpRawFromQuery" href="${buildRawLogHref(`line:${line}`)}" data-line="${line}">${line}</a>`).join(", ")}</span>`
        : "";
      return {
  ...
  if (triageHighlightsPanel) {
    triageHighlightsPanel.hidden = issues.length === 0 && namespaces.length === 0;
  }
  ...
  const executionTimelineRows = traceEvents
    .map((event) => {
      ...
      const line = rawLineForEvidence(event?.raw || event?.evidence?.raw) || null;
  ...
  const filteredExecutionTimelineRows = executionTimelineRows.filter((item) =>
    !item.isInternal
      && (selectedExecutionEventCategory === EXECUTION_EVENT_CATEGORY_ALL
        || getExecutionEventCategory(item.eventType) === selectedExecutionEventCategory),
  );

  if (executionStoryTable) {
    if (filteredExecutionTimelineRows.length === 0) {
      setHtml(executionStoryTable, "<p>No execution steps matched the selected event types.</p>");
    } else {
      let lastPhaseGroup = null;
      const rowsHtml = filteredExecutionTimelineRows.map((item) => {
        let groupHeader = "";
        if (!item.isInternal && item.phaseGroup !== lastPhaseGroup) {
          lastPhaseGroup = item.phaseGroup;
          groupHeader = `<tr class="storyPhaseHeaderRow" data-phase-group="${escapeHtml(item.phaseGroup)}"><td colspan="4">${escapeHtml(item.phaseGroup)}</td></tr>`;
        }
        const rowClass = ` data-event-type="${escapeHtml(item.eventType)}"`;
        return `${groupHeader}<tr${rowClass}><td><span class="storyEventTypeBadge">${escapeHtml(item.eventType)}</span></td><td class="storyNameCell">${escapeHtml(item.name)}</td><td>${escapeHtml(formatDurationMs(item.durationMs))}</td><td>${queryLineLink(item.line)}</td></tr>`;
      }).join("");

      setHtml(executionStoryTable, `<table><thead><tr><th>Event Type</th><th>Name</th><th>Duration</th><th>Line #</th></tr></thead><tbody>${rowsHtml}</tbody></table>`);
  ...
  if (executionInsightsPanel) {
    executionInsightsPanel.hidden = executionTimelineRows.length === 0 && executionHotspots.length === 0;
  }


  const soqlRowsAll = database?.soql || [];
  const soqlPatterns = Array.isArray(database?.soqlPatterns) ? database.soqlPatterns : [];
  ...
  for (const pat of loopSuspectPatterns) {
    if (Array.isArray(pat.queryIds)) pat.queryIds.forEach(id => loopSuspectQueryIds.add(id));
  }

  if (n1WarningContainer) {
    if (loopSuspectPatterns.length > 0) {
      const sorted = [...loopSuspectPatterns].sort((a,b) => (b.executionCount || b.count || 0) - (a.executionCount || a.count || 0));
      const top = sorted[0];
      if (top) {
        setHtml(n1WarningContainer, `
  ...
  const problematic = [...soqlRowsAll]
    .sort((a, b) => (Number(b?.durationMs || 0) - Number(a?.durationMs || 0)) || (Number(b?.rows || 0) - Number(a?.rows || 0)))
    .slice(0, uiConfig.limits.soql);
  const allChronological = [...soqlRowsAll].sort((a, b) =>
    Number(a?.evidence?.timestampNs || 0) - Number(b?.evidence?.timestampNs || 0),
  );
  ...
  const renderSoqlRow = (q) => {
    const rawLine = rawLineForEvidence(q?.evidence?.raw);
    const lineCell = Number.isFinite(Number(rawLine))
      ? `<a href="${buildRawLogHref(`line:${rawLine}`)}" class="jumpRawFromQuery" data-line="${rawLine}">${rawLine}</a>`
      : "-";
    const nameText = String(q?.query || "-").replace(/\s+/g, " ").trim();
    const isN1 = loopSuspectQueryIds.has(q.id);
    const n1Badge = isN1 ? ' <span class="n1Badge" title="N+1 Loop Suspect" style="background:var(--warn-background);color:var(--warn-accent);padding:2px 4px;border-radius:3px;font-size:0.8em;margin-left:6px;border:1px solid var(--warn-border);">N+1</span>' : '';
    return [
  ...
  setHtml(problematicQueriesTable, table(
    ["Name", "Rows", "Duration", "Line #"],
    problematic.map((q) => renderSoqlRow(q))
  ).replace('<table>', '<table class="queryTable">'));
  setHtml(allQueriesTable, table(
    ["Name", "Rows", "Duration", "Line #"],
  ...
  if (queriesHeading) setHtml(queriesHeading, '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="section-icon"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>SOQL Queries');
  ...
  const slowestQuery = [...soqlRowsAll]
    .sort((a, b) => Number(b?.durationMs || 0) - Number(a?.durationMs || 0))[0] || null;
  const structuredSavepoints = Array.isArray(report?.savepoints) ? report.savepoints : [];

  const dmlRowsAll = database?.dml || [];
  allDmlCount = dmlRowsAll.length;
  const dmlRowsVisible = dmlExpanded ? dmlRowsAll : dmlRowsAll.slice(0, uiConfig.limits.dml);
  const dmlTableHtml = table(
    ["Object", "Operation", "Rows", "Duration", "Line #"],
    dmlRowsVisible.map((d) => {
      const rawLine = rawLineForEvidence(d?.evidence?.raw);
      const lineCell = Number.isFinite(Number(rawLine))
        ? `<a href="${buildRawLogHref(`line:${rawLine}`)}" class="jumpRawFromQuery" data-line="${rawLine}">${rawLine}</a>`
        : "-";
  ...
  setHtml(dmlTable, dmlTableHtml.replace('<table>', '<table class="dmlTable">'));
  ...
  if (savepointList) {
    if (structuredSavepoints.length > 0) {
      ...
      setHtml(savepointList, table(
        ["Marker", "Name", "Line #"],
        structuredSavepoints.map((sp) => {
          const markerType = sp.type === "rollback" ? "SAVEPOINT_ROLLBACK" : "SAVEPOINT_SET";
          const rawRow = sp.rawLine ? rawLineForEvidence(sp.rawLine) : null;
          const lineCell = Number.isFinite(Number(rawRow))
            ? `<a href="${buildRawLogHref(`line:${rawRow}`, "#evidence")}" class="jumpRawFromQuery" data-line="${rawRow}">${rawRow}</a>`
            : "–";
          
          let markerName = sp.name || "–";
          const lineSuffixIdx = markerName.indexOf(", line:");
          if (lineSuffixIdx > 0) markerName = markerName.substring(0, lineSuffixIdx).trim();

          return [
            escapeHtml(markerType),
            escapeHtml(markerName),
            lineCell,
          ];
        }),
      ));
      ...
      setHtml(savepointList, "<p>No savepoints or rollback markers were detected.</p>");
    }
  }
  ...
  const parserWarningItems = parserWarningsExpanded ? parserWarningItemsRaw : parserWarningItemsRaw.slice(0, 8);
  ...
  if (diagnosticsPanel) {
    setHtml(diagnosticsPanel, buildRichDiagnosticsMarkup(report, window.diagState));
...
document.addEventListener("click", (e) => {
    ...
    (async () => {
      ...
      try {
        ...
        if (redactionSettings?.enabled) {
          toCopy = redactLines(String(text).split(/\r?\n/), redactionSettings).join("\n");
        }
      } catch {
  ...
  const rawLineNumber = Number(button.dataset.prettyLine || row.dataset.rawLineNumber);
  
  if (isOpen) {
    prettyCard.setAttribute("hidden", "");
    setPrettyToggleState(button, false);
    if (Number.isFinite(rawLineNumber) && rawLineNumber > 0) expandedRawLineNumbers.delete(rawLineNumber);
    return;
  }
  ...
  if (kind === "range") {
    ...
    const from = dashIdx > 0 ? Math.max(1, Number(rangeStr.slice(0, dashIdx))) : 0;
    const to = dashIdx > 0 ? Math.min(currentRawLogLines.length, Number(rangeStr.slice(dashIdx + 1))) : 0;
    if (from > 0 && to >= from) {
      const lines = [];
      const hardEnd = Math.min(to, from + PRETTY_MAX_RANGE_LINES - 1);
      for (let i = from; i <= hardEnd; i += 1) lines.push(`${i}: ${currentRawLogLines[i - 1] || ""}`);
  ...
  if (Number.isFinite(rawLineNumber) && rawLineNumber > 0) expandedRawLineNumbers.add(rawLineNumber);
});

document.addEventListener("change", (e) => {
  const select = e.target.closest("#executionTypeFilterSelect");
  if (!select) return;
  selectedExecutionEventCategory = String(select.value || EXECUTION_EVENT_CATEGORY_ALL);
  if (currentReportData) {
    render(currentReportData, currentRawLogLines);
    renderRawLogSearchResults(rawSearchInput.value);
  }
...
function buildRichDiagnosticsMarkup(report, state = {}) {
  ...
  const lineLink = (item) => {
    const ev = item?.evidence || {};
    const n = rawLogLineFromEvidence(ev) ||
      (Array.isArray(ev.rawLogLineNumbers) ? ev.rawLogLineNumbers.map(Number).find(x => x > 0) : null);
    if (!n) return "-";
    return `<a class="reportEvidenceLink jumpRawFromQuery" href="#evidence?q=${encodeURIComponent(`line:${n}`)}" data-evidence-line="${n}">${n}</a>`;
  };
  ...
  const takeVisible = (items, expanded, limit) => expanded ? items : items.slice(0, limit);
  ...
  const execCtxPanel = execCtx ? (() => {
    ...
    const overrideBadge = isOverride
      ...
      : "";
    const signalsList = Array.isArray(execCtx.signals) && execCtx.signals.length > 0
    ...
    <section class="panel">
      ...
      <div style="padding-top: 16px;">
        ${visibleStructural.length === 0
          ...
          : issueTableHtml(visibleStructural.map(item => ({ ...item, severity: "warn" })), "Warning", "Detail")}
      </div>
    </section>
...
window.toggleDiagSection = function(key) {
  if (key === 'issues') window.diagState.issuesExpanded = !window.diagState.issuesExpanded;
  if (key === 'structural') window.diagState.structuralExpanded = !window.diagState.structuralExpanded;
  const diagnosticsPanel = document.getElementById("diagnosticsPanel");
  if (diagnosticsPanel && currentReportData) {
    setHtml(diagnosticsPanel, buildRichDiagnosticsMarkup(currentReportData, window.diagState));
    bindDiagButtons(diagnosticsPanel);
  }
};
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!settled) {
        settled = true;
        worker.terminate();
        reject(new Error("Parse timed out after 120 seconds. The log file may be too large."));
      }
    }, 120_000);
...
async function initializeViewer() {
  ...
  try {
    ...
    if (!logFileName) {
      setViewerReady();
      alert("No log file specified. Use ?log=<filename> in the URL.");
      return;
    }

    const logUrl = `/logs/${encodeURIComponent(logFileName)}`;
    const res = await fetch(logUrl, { cache: "no-store" });
    if (!res.ok) throw new Error(`Could not fetch log file: ${logFileName} (HTTP ${res.status})`);
    ...
    if (report && typeof report === "object") {
      ...
      report.source.input.fileName = report.source.input.fileName || logFileName;
    }

    currentReportUrl = logFileName;
    currentRawLogLines = rawLogLines;
    prettyPayloadCache.clear();
    render(report, currentRawLogLines);
...
if (rawUserDebugOnlyToggle) {
  rawUserDebugOnlyToggle.addEventListener("change", () => {
    ...
    if (userDebugOnly) {
      ...
      if (rawContextSelect) rawContextSelect.disabled = isErrorsOnlyMode() || Boolean(rawShowAllToggle?.checked);
    }
    saveLogExplorerSettings({
      contextRows: getSelectedContextRows(),
      userDebugOnly,
...
if (rawErrorsToggle) {
  rawErrorsToggle.addEventListener("change", () => {
    ...
    if (errorsOnly) {
      ...
      if (rawContextSelect) rawContextSelect.disabled = isUserDebugOnlyMode() || Boolean(rawShowAllToggle?.checked);
    }
    saveLogExplorerSettings({
      contextRows: getSelectedContextRows(),
      userDebugOnly: isUserDebugOnlyMode(),
...
if (rawShowAllToggle) {
  rawShowAllToggle.addEventListener("change", () => {
    ...
    // above). Just re-render so the empty-query default (100 lines) kicks in.
    renderRawLogSearchResults(rawSearchInput.value);
  });
}
...
(async () => {
  ...
  if (redactNameListArea) redactNameListArea.value = (s.nameList || []).join("\n");
...
if (redactSaveViewerBtn) {
  redactSaveViewerBtn.addEventListener("click", async () => {
    ...
    if (redactViewerStatus) {
      ...
      setTimeout(() => { redactViewerStatus.textContent = ""; }, 2000);
    }
  });
}
...
document.addEventListener("click", (e) => {
  const scopeIdLink = e.target instanceof Element ? e.target.closest(".scopeGroupIdLink") : null;
  if (scopeIdLink) {
...
[triageSummaryTabLink, executionStoryTabLink, dataLimitsTabLink, diagnosticsTabLink, logExplorerTabLink, overviewTabLink].forEach((link) => {
  link?.addEventListener("click", (event) => {
    event.preventDefault();
...
async function init() {
  ...
  if (rawUsageHint && !rawUsageHint.textContent.trim()) {
    rawUsageHint.textContent = buildAdaptiveRefineHint(currentRawLogLines.length);
  }
