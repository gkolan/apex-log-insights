...
3. Run the code, dependency, package, and runtime-cycle gates:
   ...
   A stable release requires all four commands to finish successfully, `pnpm audit:report` to write zero issues, and `pnpm bugs:report` to write zero issues. The bug report checks built package bundles, so run it after `pnpm build`.
