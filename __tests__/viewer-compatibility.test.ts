...
it('extension and viewer version markers stay in sync with package metadata', async () => {
  ...
  const manifest = JSON.parse(await readFile(resolve(root, 'packages/browser-ext/manifests/chrome/manifest.json'), 'utf8'));
  const viewerApp = await readFile(resolve(root, 'viewer/app.js'), 'utf8');
  const extensionApp = await readFile(resolve(root, 'packages/browser-ext/shared/app.js'), 'utf8');

  const version = String(pkg.version);
  const viewerVersion = viewerApp.match(/^const APP_VERSION = "([^"]+)";/m)?.[1] ?? null;
  const extensionVersion = extensionApp.match(/^const APP_VERSION = "([^"]+)";/m)?.[1] ?? null;
  const extensionHeaderVersion = extensionApp.match(/^\s*\* Version: ([^\s]+)$/m)?.[1] ?? null;
...
it('committed fixture logs normalize and render across the rewritten views', async () => {
  ...
  for (const name of fixtureNames) {
    ...
    expect(triageHtml, `${name}: triage render failed`).toMatch(/What happened/);
    expect(executionHtml, `${name}: execution render failed`).toMatch(/What ran, in what order/);
    expect(dataHtml, `${name}: data render failed`).toMatch(/What touched the database/);
    expect(diagnosticsHtml, `${name}: diagnostics render failed`).toMatch(/What is suspicious/);
    expect(evidenceHtml, `${name}: evidence render failed`).toMatch(/Log Explorer/);
  }
});
...
it('execution story timeline source remains trace.events (not span row synthesis)', async () => {
  ...
  expect(appJs).toMatch(/const traceEvents = Array\.isArray\(execution\?\.trace\?\.events\)/);
...
it('extension enforces a strict report schema guard for worker payload', async () => {
  const extensionOverlay = await readFile(resolve(process.cwd(), 'packages/browser-ext/shared/app-extension-only.js'), 'utf8');
  expect(extensionOverlay).toMatch(/payload\.report\?\.reportVersion === "3\.0\.0"/);
});
