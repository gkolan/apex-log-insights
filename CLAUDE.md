...
Use this file to orient Claude Code and other coding agents before they inspect or change the repository. The goal is to send each change to the correct layer, preserve generated-file boundaries, and require the same tests and documentation updates expected from a human contributor.
...
# ── Quality ──
pnpm audit:report               # security scan (deps + secrets + eslint-security)
pnpm bugs:report                # bug scan (knip + tsc strict + attw + madge)
...
`Added`, `Changed`, `Fixed`, `Removed`, `Breaking`. Release notes are derived from the
