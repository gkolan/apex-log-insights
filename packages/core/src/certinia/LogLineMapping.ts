...
import {
  AsyncApexEndLine,
  AutomationSpanBeginLine,
  AutomationSpanEndLine,
  BasicExitLine,
  BasicLogLine,
  BatchApexExecuteBeginLine,
  BulkDMLEntry,
  ...
  CursorCreateBeginLine,
  CursorCreateEndLine,
  CursorFetchLine,
  CursorFetchPageLine,
  DataWeaveUserDebugLine,
  DMLBeginLine,
  DMLEndLine,
  DMLErrorLine,
  ...
  ExitingManagedPackageLine,
  EventServicePubBeginLine,
  EventServicePubDetailLine,
  EventServicePubEndLine,
  EventServiceSubBeginLine,
  EventServiceSubDetailLine,
  EventServiceSubEndLine,
  ExceptionThrownLine,
  ...
  FLSViolationLine,
  FormulaEvaluateBeginLine,
  FutureMethodBeginLine,
  HeapAllocateLine,
  HeapDeallocateLine,
...
/**
 * Returns the {@link LogEvent} subclass constructor for a given log event type.
 *
 * Used during parsing to instantiate the correct event class for each line
 * in the debug log. Returns `null` for unknown or empty event types.
 *
 * @param eventName - The Salesforce log event type string (e.g. `'CODE_UNIT_STARTED'`).
 * @returns The constructor for the matching LogEvent subclass, or `null`/`undefined`
 *   if no mapping exists for the given event type.
 */
...
/**
 * Read-only map from every supported {@link LogEventType} string to its
 * corresponding {@link LogEvent} subclass constructor.
 *
 * This is the canonical registry of all log event types the parser
 * understands. {@link getLogEventClass} delegates to this map.
 */
