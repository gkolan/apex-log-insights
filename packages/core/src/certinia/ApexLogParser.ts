...
export class ApexLogParser {
  ...
  private parseLine(line: string, lastEntry: LogEvent | null): LogEvent | null {
    ...
    if (!hasType && lastEntry?.acceptsText) {
      // wrapped text from the previous entry? Cap at 100KB to prevent memory exhaustion.
      if (lastEntry.text.length < 100_000) {
        lastEntry.text += '\n' + line;
      } else if (lastEntry.text.length === 100_000) {
        this.addLogIssue(
          lastEntry.timestamp,
          'Text-Truncation',
  ...
  private parseTree(currentLine: LogEvent, lineIter: LineIterator, stack: LogEvent[], depth: number = 0) {
    const MAX_DEPTH = 500;
    if (depth >= MAX_DEPTH) {
      // Treat as leaf node at max depth to prevent stack overflow
      currentLine.exitStamp = currentLine.timestamp;
      return;
    }
    // Warn on non-monotonic timestamps that indicate log corruption or truncation
