...
export type { FlatEvent, UnknownRecord, ParsedVariableAssignment, ParsedVariableScope, ParsedExplainPlan, DatabaseSoqlEntry, DatabaseDmlEntry, DatabaseCalloutEntry, ExecutionContextDetection };
 ...
 * @example
 * ```ts
 * import { parse, buildInsightsReport } from '@apex-log-insights/core';
 *
...
export function buildInsightsReport(params: {
  ...
  const namespaces = asArray(readPath(parserResult, ['namespaces'])).filter((v): v is string => typeof v === 'string');
  const governorLimits = readPath(parserResult, ['governorLimits']) ?? {};

  const startNs = asNumber(readPath(parserResult, ['startTime'])) ?? 0;
  const endNs = asNumber(readPath(parserResult, ['executionEndTime'])) ?? 0;
  ...
  if (firstEvent && lastEvent) {
    const firstNs = asNumber(firstEvent.timestampNs) ?? 0;
    const lastNs = asNumber(lastEvent.timestampNs) ?? 0;
    if (lastNs > firstNs) {
      durationMs = Math.max(0, Math.round((lastNs - firstNs) / 1_000_000));
    }
  }
  // Fallback to parser values if event timestamps didn't yield a result
  if (durationMs === 0 && endNs > startNs) {
    durationMs = Math.max(0, Math.round((endNs - startNs) / 1_000_000));
  }
  ...
  const allTriggerNames = Array.from(
    new Set(
      allEvents
        .filter((e) => e.type === 'CODE_UNIT_STARTED' && e.text.includes('trigger event'))
  ...
  const traceEvents = allEvents.map((e, i) => {
    ...
    // Pre-compute durationMs the same way database.soql / database.dml do, with
    // a fallback to endNs − timestampNs so the viewer never needs to guess.
    let durationMs: number | null = null;
    if (durationNs !== null && durationNs > 0) {
      durationMs = Math.round((durationNs / 1_000_000) * 1000) / 1000;
    } else if (endNs !== null && e.timestampNs !== null && endNs > e.timestampNs) {
      durationMs = Math.round(((endNs - e.timestampNs) / 1_000_000) * 1000) / 1000;
    }
    return {
      id: `event-${i + 1}`,
      idx: e.idx,
  ...
  const _cumulativeProfiling = collectCumulativeProfilingSections(allEvents);
  ...
  // Consume responses in order so each is matched to at most one request.
  // Use strict < for the upper bound to avoid the same response matching two
  // requests when a response timestamp equals the next request's timestamp.
  const availableResponses = allEvents
    .filter((e) => e.type === 'CALLOUT_RESPONSE')
  ...
  const databaseCallouts: DatabaseCalloutEntry[] = calloutEvents.map((e, i) => {
    ...
    // Find the first unconsumed CALLOUT_RESPONSE in [requestTs, nextRequestTs)
  ...
  const spans = spanCandidates.map((event, i) => {
    ...
    return {
      ...
      durationMs: event.durationTotalNs !== null ? Math.round((event.durationTotalNs / 1_000_000) * 1000) / 1000 : null,
      selfDurationNs: event.durationSelfNs,
      selfDurationMs: event.durationSelfNs !== null ? Math.round((event.durationSelfNs / 1_000_000) * 1000) / 1000 : null,
      evidence: {
  ...
  const validationBlocks = validationUnits.map((unit) => {
    ...
    const eventsInWindow = allEvents
      .filter((event) => event.timestampNs >= startNs && event.timestampNs <= endNs)
  ...
  const flowBlocks = flowUnits.map((unit) => {
    ...
    return {
      ...
      totals: {
        ...
        errorCount: eventsInWindow.filter((event) => event.type.includes('ERROR') || event.type.includes('FAULT'))
          .length,
      },
  ...
  const workflowBlocks = workflowUnits.map((unit) => {
    ...
    const eventsInWindow = allEvents
      .filter((event) => event.timestampNs >= startNs && event.timestampNs <= endNs)
      .filter((event) => event.type.startsWith('WF_') || event.type === 'EVENT_SERVICE_PUB_BEGIN')
      .sort((a, b) => a.timestampNs - b.timestampNs);
  ...
  const lineToEventIds: Record<string, string[]> = {};
  ...
  for (let i = 0; i < traceEvents.length; i += 1) {
    ...
    const endNs = Number(sourceEvent?.endTimestampNs ?? sourceEvent?.timestampNs ?? 0);
  ...
  const rawEventErrors = allEvents
    .filter((event) => isErrorEventType(event.type))
    .map((event) => {
      const summary = event.text || event.type;
      // Only include a description when the event text has additional detail
      // beyond the summary (e.g. multi-line stack traces). The raw log line
      // is already linked via evidence — repeating it as description is noise.
  ...
  const issueErrors = issues.map((issue) => ({
    ...
    namespace: 'default',
    evidence: {
      lineNumber: asNumber(readPath(issue, ['lineNumber'])) ?? null,
      timestampNs: null,
    },
  }));
  ...
  const dmlEventRows = databaseDml
    .filter((dml) => dml.source === 'event')
    .reduce((sum, dml) => sum + (dml.rows ?? 0), 0);
  const dmlByObject = databaseDml
    .filter((dml) => dml.source === 'event')
    .reduce<Record<string, number>>((acc, dml) => {
      const key = dml.sObject || dml.operation || 'Unknown';
      acc[key] = (acc[key] ?? 0) + (dml.rows ?? 0);
      return acc;
    }, {});

  const executionContext = (contextOverride ? buildContextOverride(contextOverride) : null)
  ...
  for (const nc of namedCredentials) {
    ...
    const paired = databaseCallouts.find(
      (c) =>
        !pairedCalloutIds.has(c.id) &&
        (c.evidence.timestampNs ?? -1) >= ncTs &&
        (c.evidence.timestampNs ?? -1) < ncTs + NC_PAIR_WINDOW_NS,
    );
    if (paired) {
  ...
  const syntheticWarnings: Array<{
    ...
    evidence: {
      lineNumber: number | null;
      lineNumbers?: number[];
      /** Raw log line texts for each occurrence — looked up by the viewer to resolve actual row numbers. */
  ...
  for (const query of databaseSoql) {
    ...
    const queryType = isEmptyResult && isSlow
      ? 'SOQL_QUERY_WARNING'
      : isEmptyResult
        ? 'EMPTY_QUERY_WARNING'
        : isSlow
  ...
  const soql = isRecord(governorLimits) ? readPath(governorLimits, ['soqlQueries']) : undefined;
  const rows = isRecord(governorLimits) ? readPath(governorLimits, ['queryRows']) : undefined;
  const dmlStatements = isRecord(governorLimits) ? readPath(governorLimits, ['dmlStatements']) : undefined;
  const dmlRows = isRecord(governorLimits) ? readPath(governorLimits, ['dmlRows']) : undefined;
  const cpu = isRecord(governorLimits) ? readPath(governorLimits, ['cpuTime']) : undefined;
  const heap = isRecord(governorLimits) ? readPath(governorLimits, ['heapSize']) : undefined;
  const callouts = isRecord(governorLimits) ? readPath(governorLimits, ['callouts']) : undefined;
  const queueables = isRecord(governorLimits) ? readPath(governorLimits, ['queueableJobsAddedToQueue']) : undefined;
  const emailInvocations = isRecord(governorLimits) ? readPath(governorLimits, ['emailInvocations']) : undefined;
  const futureCalls = isRecord(governorLimits) ? readPath(governorLimits, ['futureCalls']) : undefined;
  ...
  const snapshotsRaw = isRecord(governorLimits) ? asArray(readPath(governorLimits, ['snapshots'])) : [];
  ...
  const spanHotspots = [...spans]
    ...
    .map((span) => ({
      ...
      lineNumber: isRecord(span.evidence) ? asNumber(span.evidence.lineNumber) ?? null : null,
  ...
  const queueableHotspots = queueableBeginEvents.map((queueableEvent, i) => {
    ...
    return {
      ...
      durationMs: queueableEvent.durationTotalNs !== null
        ? Math.round((queueableEvent.durationTotalNs / 1_000_000) * 1000) / 1000
        : null,
  ...
  const queueableByLabel = new Map<string, (typeof queueableHotspots)[number]>();
  ...
  const hotspots = [...queueableByLabel.values(), ...spanHotspots]
    .sort((a, b) => (a.startNs ?? Number.MAX_SAFE_INTEGER) - (b.startNs ?? Number.MAX_SAFE_INTEGER))
    .slice(0, 25);
  ...
  // Re-sort errorItems with new warnings included
  const allErrorItems = [...issueErrors, ...parseErrors, ...rawEventErrors, ...syntheticWarnings]
    .sort((a, b) => {
  ...
  return {
    ...
    overview: {
      ...
      topMetrics: {
        ...
        soql: {
          count: soqlUsed !== undefined && soqlUsed > 0 ? soqlUsed : databaseSoql.length,
          rows:
            soqlRowsUsed !== undefined && soqlRowsUsed > 0
              ? soqlRowsUsed
              : databaseSoql.reduce((sum, q) => sum + (q.rows ?? 0), 0),
        },
        sosl: {
        ...
        dml: {
          statements: dmlStatementsUsed !== undefined && dmlStatementsUsed > 0 ? dmlStatementsUsed : databaseDml.length,
          rows: dmlRowsUsed !== undefined && dmlRowsUsed > 0 ? dmlRowsUsed : databaseDml.reduce((sum, d) => sum + (d.rows ?? 0), 0),
        },
        callouts: {
    ...
    limits: {
      cumulative: {
        defaultNamespace: {
          soqlQueries: toLimit(asNumber(readPath(soql, ['used'])), asNumber(readPath(soql, ['limit']))),
          soqlRows: toLimit(asNumber(readPath(rows, ['used'])), asNumber(readPath(rows, ['limit']))),
          dmlStatements: toLimit(
            asNumber(readPath(dmlStatements, ['used'])),
            asNumber(readPath(dmlStatements, ['limit'])),
          ),
          dmlRows: toLimit(asNumber(readPath(dmlRows, ['used'])), asNumber(readPath(dmlRows, ['limit']))),
          cpuTimeMs: toLimit(asNumber(readPath(cpu, ['used'])), asNumber(readPath(cpu, ['limit']))),
          heapBytes: toLimit(asNumber(readPath(heap, ['used'])), asNumber(readPath(heap, ['limit']))),
          callouts: toLimit(asNumber(readPath(callouts, ['used'])), asNumber(readPath(callouts, ['limit']))),
          queueables: toLimit(asNumber(readPath(queueables, ['used'])), asNumber(readPath(queueables, ['limit']))),
        },
      },
    ...
    database: {
      ...
      integrationOperations: cloneJsonLike(integrationOperations) as unknown as JsonValue,
    ...
    systemModeTransitions: cloneJsonLike(systemModeTransitions) as unknown as JsonValue,
    ...
    managedPackageImpact: cloneJsonLike(managedPackageImpact) as unknown as JsonValue,
    mixedDmlAnalysis: cloneJsonLike(mixedDml) as unknown as JsonValue,
    recursiveTriggerAnalysis: cloneJsonLike(recursiveTriggerDetection) as unknown as JsonValue,
    errors: {
