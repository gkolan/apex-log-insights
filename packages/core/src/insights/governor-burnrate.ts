...
export function buildGovernorBurnRate(
  ...
  for (const snap of snapshotsRaw) {
    ...
    trajectory.push({
      timestampNs: ts,
      namespace: ns,
      soqlUsed: isRecord(soqlQ) ? asNumber((soqlQ as UnknownRecord).used) ?? null : null,
      soqlRowsUsed: isRecord(queryRows) ? asNumber((queryRows as UnknownRecord).used) ?? null : null,
      dmlUsed: isRecord(dmlS) ? asNumber((dmlS as UnknownRecord).used) ?? null : null,
      dmlRowsUsed: isRecord(dmlR) ? asNumber((dmlR as UnknownRecord).used) ?? null : null,
      cpuUsed: isRecord(cpuT) ? asNumber((cpuT as UnknownRecord).used) ?? null : null,
      heapUsed: isRecord(heapS) ? asNumber((heapS as UnknownRecord).used) ?? null : null,
      calloutsUsed: isRecord(callouts) ? asNumber((callouts as UnknownRecord).used) ?? null : null,
      futureCallsUsed: isRecord(futureCalls) ? asNumber((futureCalls as UnknownRecord).used) ?? null : null,
      queueablesUsed: isRecord(queueables) ? asNumber((queueables as UnknownRecord).used) ?? null : null,
    });
  }

  trajectory.sort((a, b) => a.timestampNs - b.timestampNs);

  // Compute burn rates from first to last trajectory point
  const first = trajectory[0];
  const last = trajectory[trajectory.length - 1];
  const burnRates: LimitBurnRateEntry[] = [];

  if (first && last && isRecord(governorLimits)) {
    const elapsedSec = Math.max(0.001, (last.timestampNs - first.timestampNs) / 1_000_000_000);
    ...
    for (const def of limitDefs) {
      ...
      const max = isRecord(limObj) ? asNumber((limObj as UnknownRecord).limit) ?? null : null;
      const used = def.lastUsed;
      const pctUsed = used !== null && max !== null && max > 0
      ...
      if (def.firstUsed !== null && def.lastUsed !== null) {
        ...
        if (max !== null && burnRatePerSec > 0) {
          ...
          projectedHeadroom = Number.isFinite(raw) ? Math.min(Math.round(raw * 1000) / 1000, 999999) : null;
        }
      }
  ...
  const byNamespace: GovernorBurnRate['byNamespace'] = {};
  ...
  for (let i = 0; i < totalPhases; i++) {
    ...
    // Find the closest trajectory point at or after the phase end
    ...
    const criticalPcts = [soqlPct, soqlRowsPct, dmlPct, cpuPct, heapPct, calloutsPct, futureCallsPct, queueablesPct]
