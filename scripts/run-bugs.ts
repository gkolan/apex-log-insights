...
{
  const raw = run('npx knip --reporter json 2>/dev/null');
  ...
  if (parsed && typeof parsed === 'object') {
    const categoryMap: Record<string, { label: string; severity: string }> = {
      ...
      optionalPeerDependencies: { label: 'Unused optional peer dependency', severity: 'low' },
    };
    ...
    const emitCategoryItems = (
      ...
      const meta = categoryMap[category] || { label: category, severity: 'medium' };

      for (const item of items) {
        knipCount++;
        const issue = (item && typeof item === 'object') ? (item as KnipIssue) : {};
        const filePath = issue.name || issue.path || fallbackFile || String(item);
        const relPath = filePath.replace(root + '/', '');
        const symbols = issue.symbols
          ? issue.symbols.map((s: { symbol: string }) => `\`${s.symbol}\``).join(', ')
          : null;
        ...
        if (fallbackFile && fallbackFile !== filePath) {
          bodyParts.push(`**Reported in:** \`${fallbackFile.replace(root + '/', '')}\``);
        }
        if (symbols) bodyParts.push(`**Symbols:** ${symbols}`);
        if (issue.line) bodyParts.push(`**Line:** ${issue.line}`);
        bodyParts.push('', `**Action:** Review and remove if truly unused, or add to knip ignore config if intentional.`);
    ...
    if (fileIssues) {
      for (const fileIssue of fileIssues) {
        ...
        const fallbackFile = typeof issueRecord.file === 'string' ? issueRecord.file : undefined;
    ...
    if (lines.some((l) => /unused|unlisted|unresolved/i.test(l))) {
      for (const line of lines) {
        ...
        if (!trimmed || trimmed.startsWith('---') || trimmed.startsWith('knip')) continue;
  ...
  for (const pkg of packages) {
    ...
    for (const line of errorLines) {
      ...
      if (match) {
        ...
        writeIssue('tsc-unchecked-index', 'medium', `tsc: ${line.slice(0, 80)}`, line);
      }
    }
  }
  if (tscCount === 0) {
  ...
  for (const pkg of publishable) {
    ...
    if (problemLines.length > 0) {
      for (const line of problemLines) {
        attwCount++;
        const trimmed = line.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '').trim();
...
{
  // Scan compiled JS (dist/) when available to avoid false positives from
  // TypeScript `import type` statements, which are erased at build time.
  // Falls back to source for non-compiled targets like the viewer.
  const targets = [
    { label: 'packages/core', dir: 'packages/core/src', ext: 'ts' },
    { label: 'packages/cli', dir: 'packages/cli/src', ext: 'ts' },
    { label: 'packages/mcp', dir: 'packages/mcp/src', ext: 'ts' },
    ...
    for (const line of circularLines) {
      ...
      if (target.ext === 'ts') {
        ...
        for (let i = 0; i < files.length && isTypeOnly; i++) {
          ...
          try {
            const content = execSync(`cat "${importerPath}"`, { encoding: 'utf8', cwd: root });
