 ...
 *
 * 1. Optionally sets a requested version in package.json, manifests, and viewer assets
 ...
 *   node --import tsx scripts/export-extension.ts --version minor  — bump minor and rebuild
 *   node --import tsx scripts/export-extension.ts --version 2.1.0  — explicit version
 *   node --import tsx scripts/export-extension.ts --ui-only        — skip worker rebuilds
 */
...
async function main(): Promise<void> {
  ...
  function bumpVersion(current: string, bump: string): string {
    if (/^\d+\.\d+\.\d+$/.test(bump)) {
      ...
      if (parts.some((p) => !Number.isFinite(p) || p < 0 || p > 65535)) {
        throw new Error(
          `Version components must be integers 0–65535, got: ${bump}`,
        );
      }
      return bump;
    }
    ...
    throw new Error(
      `Unknown version bump type: "${bump}". Use major, minor, patch, or an explicit version like 1.2.3`,
    );
  }
  ...
  if (versionArgIdx !== -1 && !args[versionArgIdx + 1]) {
    throw new Error(
      "--version requires major, minor, patch, or an explicit version such as 2.1.0",
    );
  }
  const versionBump =
    versionArgIdx !== -1 ? args[versionArgIdx + 1] : undefined;
  ...
  const newVersion = versionBump
    ? bumpVersion(oldVersion, versionBump)
    : oldVersion;

  console.log(
    `\n[export] Building version ${newVersion}${versionBump ? ` (${oldVersion} → ${newVersion})` : ""}\n`,
  );
  ...
  writeFileSync(
    appJsPath,
    readFileSync(appJsPath, "utf8").replace(
      /^const APP_VERSION = "[^"]*";/m,
  ...
  for (const browser of ["chrome", "edge", "firefox"]) {
    ...
    if (existsSync(mfPath)) {
      ...
      try {
        ...
        throw new Error(
          `Failed to parse ${mfPath}: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
      ...
      console.warn(
        `  [export] Warning: manifest not found for ${browser} at ${mfPath}`,
      );
    }
  }

  console.log(
    `  Versions ${versionBump ? "updated" : "synchronized"}: package.json, viewer/app.js, viewer/index.html, all manifests`,
  );
  ...
  execFileSync(
    process.execPath,
    ["--import", "tsx", "scripts/assemble-extension-ui.ts"],
    {
      cwd: root,
      stdio: "inherit",
    },
  ...
  if (existsSync(popupPath)) {
    ...
    const versionPattern = /(<span class="popupVersion">v)[^<]*/;
    if (!versionPattern.test(original)) {
      console.warn(
        "  Warning: popup.html version tag not found — HTML structure may have changed",
      );
    } else {
      const updated = original.replace(versionPattern, `$1${newVersion}`);
