 ...
 *
 * Usage:
 *   npx tsx scripts/assemble-extension-ui.ts
 */
...
function inlineModule(filename: string): string {
  ...
  if (!existsSync(filePath)) {
    throw new Error(`[assemble] Error: module ${filename} not found at ${filePath}`);
  }
  let src = readFileSync(filePath, 'utf8');
  const originalLength = src.length;
  // Remove import lines (e.g. `import { escapeHtml } from "./shared-format.js";`)
  ...
  src = src.replace(/^const DEFAULT_REDACTION_SETTINGS\s*=\s*\{[\s\S]*?\};\s*/m, '');
...
// Strip ESM import lines from the viewer code (the extension is non-module)
viewerContent = viewerContent.replace(/^import\s+.*;\s*$/gm, '').trimStart();
...
writeFileSync(resolve(extShared, 'app.js'), assembled, 'utf8');
console.log('  [assemble] shared/app.js ← viewer/app.js + modules + app-extension-only.js');
...
const stylesOverlay = existsSync(stylesOverlayPath) ? readFileSync(stylesOverlayPath, 'utf8') : '';
...
console.log('  [assemble] shared/styles.css ← viewer/styles.css + styles-extension-only.css');
