...
const packageDirs = ["core", "cli", "mcp", "browser-ext", "vscode-ext"];
...
// Update cache-busting query strings in the standalone viewer.
const viewerHtmlPath = join(root, "viewer/index.html");
let viewerHtml = readFileSync(viewerHtmlPath, "utf8");
const viewerHtmlUpdated = viewerHtml.replace(
  /(styles\.css|app\.js)\?v=[0-9.]+/g,
  `$1?v=${version}`,
);
if (viewerHtmlUpdated !== viewerHtml) {
  writeFileSync(viewerHtmlPath, viewerHtmlUpdated, "utf8");
  console.log(`  OK  viewer/index.html assets → ${version}`);
}

const serviceWorkerPath = join(root, "viewer/sw.js");
let serviceWorker = readFileSync(serviceWorkerPath, "utf8");
const serviceWorkerUpdated = serviceWorker.replace(
  /apex-log-insights-v[0-9.]+/,
  `apex-log-insights-v${version}`,
);
if (serviceWorkerUpdated !== serviceWorker) {
  writeFileSync(serviceWorkerPath, serviceWorkerUpdated, "utf8");
  console.log(`  OK  viewer service-worker cache → ${version}`);
}

// Keep the user-visible popup marker synchronized with the manifests.
const popupHtmlPath = join(root, "packages/browser-ext/shared/popup.html");
let popupHtml = readFileSync(popupHtmlPath, "utf8");
const popupHtmlUpdated = popupHtml.replace(
  /(<span class="popupVersion">v)[^<]+/,
  `$1${version}`,
);
if (popupHtmlUpdated !== popupHtml) {
  writeFileSync(popupHtmlPath, popupHtmlUpdated, "utf8");
  console.log(`  OK  extension popup marker → ${version}`);
}
...
const allFiles = [
  ...
  join(packagesDir, "vscode-ext", "package.json"),
];
...
for (const file of allFiles) {
  ...
  if (pkg.version !== version) {
    console.error(
      `  FAIL  Version mismatch in ${file}: expected ${version}, got ${pkg.version}`,
    );
    verifyFailed = true;
  }
}
